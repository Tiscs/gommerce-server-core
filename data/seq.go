...
func (s *redisSeq) Next(key string, min int64, max int64) (int64, error) {
	if v, err := lua_seq.Exec(context.Background(), s.rdb, []string{key}, []string{strconv.FormatInt(min, 10), strconv.FormatInt(max, 10)}).AsInt64(); err != nil {
		return 0, err
	} else {
		return v, nil
	}
